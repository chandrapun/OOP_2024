...
a. Change Test to test.
  = This change will produce error because filename and class name should be same. 

b. Change Emergency to emergency.
  = No error occurs here. 

c. Remove the first quotation mark in the string.
  = This change will produce a compilation error because the compiler will interpret "An Emergency Broadcast" as an identifier rather than a string literal. 
...
f. Remove the semicolon at the end of the println statement.
  = This change will produce a compilation error because semicolons are required to terminate statements in Java. 

g. Remove the last brace in the program. 
  = This change will produce a compilation error because it leaves the method main() unclosed. We should keep the brace to properly close the method and the class.
 */ 
